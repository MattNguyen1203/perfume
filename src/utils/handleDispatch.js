...
import adminProductsReducer from "../pages/admin/redux/reducers/product";
import adminOrdersReducer from "../pages/admin/redux/reducers/order";
import {
  addNewProduct,
  updateProduct,
  removeProduct,
  getAllProduct,
  detailProduct,
} from "../pages/admin/redux/reducers/product";

import {
  getAllOrders,
  getOrderDetail,
  getOrderUpdate,
} from "../pages/admin/redux/reducers/order";

const maxQuantity = 3;
const productsState = () => store.getState().adminProducts,
  productAction = (type, content) =>
    store.dispatch(adminProductsReducer.actions[type](content)),
  orderAction = (type, content) =>
    store.dispatch(adminOrdersReducer.actions[type](content));
  ...
  product: {
    form: {
      add: async function (fieldName) {
        ...
        if (item.typeInput === "object")
          item.detail.forEach((subField) => {
            info[subField.name].length < maxQuantity &&
              info[subField.name].push(0);
          });
      ...
      delete: async function (fieldName, delIndex) {
        try {
          const info = JSON.parse(
            JSON.stringify(productsState().isDetail.info)
          );
      ...
      onChange: async function (fieldName, oldVal, newVal) {
        const info = JSON.parse(JSON.stringify(productsState().isDetail.info));
        const { typeInput } = inputList.find((e) =>
          e.field.includes(fieldName)
        );
        ...
        switch (typeInput) {
          ...
          case "array":
            const indexInArr = info[fieldName].findIndex(
              (item) => item === oldVal
            );
          ...
          case "object":
            const indexInArrObj = info[fieldName].findIndex(
              (item) => item === oldVal
            );
